# gotunnel Documentation Center

**Language:** [English](./00-README.md) | [中文](../zh/00-README.md)

Welcome to the gotunnel documentation center! This contains the complete documentation for the project.

## 📚 Documentation Index

### Getting Started
- **[01-Quick Start Guide](./01-QUICKSTART.md)** - Get started with gotunnel in 5 minutes
- **[02-Configuration Guide](./02-CONFIG.md)** - Detailed configuration options
- **[03-Architecture Design](./03-ARCHITECTURE.md)** - System architecture documentation
- **[04-Protocol Documentation](./04-PROTOCOL.md)** - Communication protocol details
- **[05-Development Guide](./05-DEVELOPMENT.md)** - Developer documentation
- **[06-Troubleshooting Guide](./06-TROUBLESHOOTING.md)** - Common issues and solutions
- **[07-Design Notes](./07-DESIGN-NOTES.md)** - Design considerations and best practices
- **[08-Kubernetes Deployment](./08-K8S-DEPLOYMENT.md)** - K8s cross-network deployment guide

## Quick Navigation

- **New User:** Start with [Quick Start Guide](./01-QUICKSTART.md)
- **Developer:** Read the [Development Guide](./05-DEVELOPMENT.md), [Protocol Documentation](./04-PROTOCOL.md), and [Design Notes](./07-DESIGN-NOTES.md)
- **Deployment:** Review [Architecture Design](./03-ARCHITECTURE.md) and configuration docs

---

**You can always switch Chinese/English at the top of every doc!**

